  ...
  return (
    <div className="w-full min-h-screen flex items-center justify-center bg-gradient-to-r from-orange-300 to-pink-300">
      <div className="px-5 py-10  bg-white rounded-md shadow-lg text-black">
        ...
        <form onSubmit={formik.handleSubmit} className="transform translate-y-[-50px]">
          ...
          <div className="flex justify-center items-center mt-5">
            <button type="submit" className="uppercase border px-3 py-1 rounded-md bg-gradient-to-r from-pink-200 to-orange-200 shadow-sm">
              Sign Up {loading ? <span className="loading loading-spinner loading-xs"></span> : ""}
            </button>
          </div>
        </form>
        <hr />
        <div className="mt-5">
          <p>Note: <span className="text-[0.7rem]">Default username : admin and password :admin</span></p>
        </div>
      </div>
    </div>
  );
